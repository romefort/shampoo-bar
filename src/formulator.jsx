...
function validateFormulation(items) {
  ...
  // ── Preservative check — only require if formula contains water ──
  const hasPreservative = items.some(i => ingredientMap[i.id]?.slot === "PRESERVATIVE" && i.pct > 0);
  const formulaHasWater = items.some(i => (ingredientMap[i.id]?.waterContent || 0) > 0 && i.pct > 0);
  if (!hasPreservative && formulaHasWater) errors.push({ msg: "No preservative! Water-containing bars MUST have broad-spectrum preservation.", sev: "error" });
  if (!hasPreservative && !formulaHasWater) info.push({ msg: "No preservative — acceptable for fully anhydrous formulas. Add one if any water-bearing ingredient is introduced.", sev: "info" });

  // ── Incompatibility check ──
  const presentIds = new Set(items.filter(i => i.pct > 0).map(i => i.id));
  for (const item of items) {
    const ing = ingredientMap[item.id];
    if (!ing || item.pct === 0 || !ing.incompatible) continue;
    for (const incompId of ing.incompatible) {
      if (presentIds.has(incompId)) {
        const other = ingredientMap[incompId];
        warnings.push({ msg: `${ing.name.split("(")[0].trim()} is incompatible with ${other.name.split("(")[0].trim()} — check formulation notes.`, sev: "warn" });
      }
    }
  }

  // ── Required slot check ──
  const filledSlots = new Set(items.filter(i => i.pct > 0).map(i => ingredientMap[i.id]?.slot).filter(Boolean));
  for (const [slotId, slot] of Object.entries(FUNCTIONAL_SLOTS)) {
    // Skip PRESERVATIVE — already handled above with water-awareness
    if (slot.required && slotId !== "PRESERVATIVE" && !filledSlots.has(slotId)) {
      warnings.push({ msg: `No ${slot.name} — most syndet bars need at least one.`, sev: "warn" });
    }
  }
...
// ═══════════════════════════════════════════════════════════════
// PERSISTENCE — localStorage + URL sharing
// ═══════════════════════════════════════════════════════════════
const STORAGE_KEYS = {
  SAVED: "syndet_saved_recipes",
  DRAFT: "syndet_draft",
  PREFS: "syndet_prefs",
  PANTRY: "syndet_pantry",
};
...
const encodeRecipeToHash = (recipe) => {
  try {
    const compact = {
      n: recipe.name,
      o: recipe.notes || "",
      g: recipe.selGoal || null,
      i: recipe.items.map(x => [x.id, x.pct]),
    };
...
const decodeRecipeFromHash = (encoded) => {
  try {
    const json = decodeURIComponent(escape(atob(encoded)));
    const compact = JSON.parse(json);
    if (!compact.i || !Array.isArray(compact.i)) return null;
    const items = compact.i.map(([id, pct]) => {
      if (!ingredientMap[id]) return null;
      return { id, pct: Number(pct) };
    }).filter(Boolean);
    if (items.length === 0) return null;
    return {
      name: compact.n || "Shared Recipe",
      notes: compact.o || "",
      selGoal: compact.g || null,
      items,
    };
  } catch { return null; }
};
...
function SyndetFormulator() {
  ...
  const [batchSize, setBatchSize] = useState(() => { const p = storageGet(STORAGE_KEYS.PREFS, {}); const b = Number(p.batchSize); return b > 0 ? b : 100; });
  const [unit, setUnit] = useState(() => { const p = storageGet(STORAGE_KEYS.PREFS, {}); return UNITS[p.unit] ? p.unit : "g"; });
  const [anchorId, setAnchorId] = useState(null);
  const [selGoal, setSelGoal] = useState(null);
  // ── Recipe management ──
  const [recipeName, setRecipeName] = useState("Untitled Shampoo Bar");
  const [recipeNotes, setRecipeNotes] = useState("");
  const [savedRecipes, setSavedRecipes] = useState(() => storageGet(STORAGE_KEYS.SAVED, []));
  const [showSaved, setShowSaved] = useState(false);
  const [showPrint, setShowPrint] = useState(false);
  const [showDraftBanner, setShowDraftBanner] = useState(false);
  const [copied, setCopied] = useState(false);
  // ── Pantry ──
  const [pantry, setPantry] = useState(() => storageGet(STORAGE_KEYS.PANTRY, []));
  const [showPantry, setShowPantry] = useState(false);
  const [pantrySearch, setPantrySearch] = useState("");
  const [pantryFilterSlot, setPantryFilterSlot] = useState("ALL");
  const mountedRef = useRef(false);

  // ── Mount: load from URL hash or recover draft ──
  useEffect(() => {
    const hash = window.location.hash;
    if (hash.startsWith("#recipe=")) {
      const encoded = hash.slice(8);
      const recipe = decodeRecipeFromHash(encoded);
      if (recipe) {
        setItems(recipe.items); setRecipeName(recipe.name); setRecipeNotes(recipe.notes);
        setSelGoal(recipe.selGoal); setSelTemplate(null);
        window.history.replaceState(null, "", window.location.pathname + window.location.search);
        return;
      }
    }
    const draft = storageGet(STORAGE_KEYS.DRAFT, null);
    if (draft && draft.items && draft.items.length > 0) { setShowDraftBanner(true); }
    // Signal to auto-save effect that mount is complete (delayed to run after all initial effects)
    requestAnimationFrame(() => { mountedRef.current = true; });
  }, []);

  // ── Persist saved recipes ──
  useEffect(() => { storageSet(STORAGE_KEYS.SAVED, savedRecipes); }, [savedRecipes]);

  // ── Persist preferences ──
  useEffect(() => { storageSet(STORAGE_KEYS.PREFS, { batchSize, unit }); }, [batchSize, unit]);

  // ── Persist pantry ──
  useEffect(() => { storageSet(STORAGE_KEYS.PANTRY, pantry); }, [pantry]);

  // ── Auto-save draft (1s debounce) ──
  useEffect(() => {
    if (!mountedRef.current) return;
    if (items.length === 0) { storageRemove(STORAGE_KEYS.DRAFT); return; }
    const timer = setTimeout(() => {
      storageSet(STORAGE_KEYS.DRAFT, { name: recipeName, notes: recipeNotes, items, selGoal, batchSize, unit, savedAt: new Date().toISOString() });
    }, 1000);
    return () => clearTimeout(timer);
  }, [items, recipeName, recipeNotes, selGoal, batchSize, unit]);
  ...
  const resetFormula = useCallback(() => {
    ...
    storageRemove(STORAGE_KEYS.DRAFT); setShowDraftBanner(false);
  }, []);
  ...
  const loadDraft = useCallback(() => {
    ...
    setRecipeName(draft.name || "Untitled Shampoo Bar");
    setRecipeNotes(draft.notes || "");
    setBatchSize(draft.batchSize || 100);
    setUnit(draft.unit || "g");
    setSelGoal(draft.selGoal || null);
    setSelTemplate(null);
  ...
  const setPantryAll = useCallback(() => {
    setPantry(INGREDIENTS.map(i => i.id));
  }, []);
  const setPantryClear = useCallback(() => {
    setPantry([]);
  }, []);

  // ── Share recipe via URL ──
  const shareRecipe = useCallback(() => {
    const encoded = encodeRecipeToHash({ name: recipeName, notes: recipeNotes, items, selGoal });
    if (!encoded) return;
    ...
    doCopy(url).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); });
  }, [recipeName, recipeNotes, items, selGoal]);
  ...
  const templateCompleteness = useMemo(() => {
    if (pantry.length === 0) return null;
    return TEMPLATES.map(t => {
      const total = t.ingredients.length;
      const owned = t.ingredients.filter(i => pantrySet.has(i.id)).length;
      return { id: t.id, owned, total, fraction: total > 0 ? owned / total : 0 };
    });
  }, [pantry, pantrySet]);

  const sortedTemplates = useMemo(() => {
    if (!templateCompleteness) return TEMPLATES;
    const compMap = {};
    templateCompleteness.forEach(c => { compMap[c.id] = c; });
    return [...TEMPLATES].sort((a, b) => (compMap[b.id]?.fraction || 0) - (compMap[a.id]?.fraction || 0));
    ...
    <div style={{ fontFamily: "'IBM Plex Sans', 'SF Pro Text', system-ui, sans-serif", background: C.bg, color: C.t1, height: "100vh", display: "flex", flexDirection: "column", fontSize: 13 }}>
      ...
      <div style={{ padding: "12px 20px 10px", borderBottom: `1px solid ${C.b1}`, background: C.s1 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          ...
          <button onClick={() => { setShowSaved(s => { if (!s) { setShowPantry(false); setPantrySearch(""); setPantryFilterSlot("ALL"); } return !s; }); }} style={{
          ...
          <button onClick={() => { setShowPantry(p => { if (!p) setShowSaved(false); else { setPantrySearch(""); setPantryFilterSlot("ALL"); } return !p; }); }} title="Mark ingredients you own to find makeable templates" style={{
            padding: "4px 10px", fontSize: 10, fontWeight: 600, border: `1px solid ${showPantry ? "#b87a3a" : C.b1}`,
            borderRadius: 3, background: showPantry ? "#b87a3a20" : "transparent",
            color: showPantry ? "#b87a3a" : C.t2, cursor: "pointer",
          }}>🧂 Pantry{pantry.length > 0 ? ` (${pantry.length})` : ""}</button>
          <button onClick={() => setShowPrint(true)} disabled={items.length === 0} title="View printable recipe" style={{
            padding: "4px 10px", fontSize: 10, fontWeight: 600, border: `1px solid ${C.b1}`,
            borderRadius: 3, background: "transparent", color: items.length ? C.t2 : C.t3,
            cursor: items.length ? "pointer" : "default", opacity: items.length ? 1 : 0.4,
          }}>🖨 Recipe</button>
          <button onClick={shareRecipe} disabled={items.length === 0} title="Copy shareable link" style={{
            padding: "4px 10px", fontSize: 10, fontWeight: 600, border: `1px solid ${copied ? C.acc : C.b1}`,
            borderRadius: 3, background: copied ? C.acc : "transparent",
            color: copied ? "#fff" : (items.length ? C.t2 : C.t3),
            cursor: items.length ? "pointer" : "default", opacity: items.length ? 1 : 0.4,
            transition: "all 0.2s",
      ...
      {/* Pantry drawer */}
      {showPantry && (() => {
        const pantrySlotKeys = Object.keys(FUNCTIONAL_SLOTS);
        const filteredIngredients = INGREDIENTS.filter(i => {
          if (pantryFilterSlot !== "ALL" && i.slot !== pantryFilterSlot && i.alsoSlot !== pantryFilterSlot) return false;
          if (pantrySearch) {
            const s = pantrySearch.toLowerCase();
            return i.name.toLowerCase().includes(s) || i.tags?.some(t => t.includes(s)) || i.slot.toLowerCase().includes(s);
          }
          return true;
        });
        return (
          <div style={{ padding: "10px 14px", borderBottom: `1px solid ${C.b1}`, background: C.s2, maxHeight: 240, overflow: "auto" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
              <span style={{ fontSize: 10, color: C.t3, flex: 1 }}>Check ingredients you own — templates will sort by what you can make.</span>
              <button onClick={setPantryAll} style={{
                padding: "2px 8px", fontSize: 9, fontWeight: 600, border: `1px solid ${C.b1}`,
                borderRadius: 3, background: "transparent", color: C.t2, cursor: "pointer",
              }}>All</button>
              <button onClick={setPantryClear} style={{
                padding: "2px 8px", fontSize: 9, fontWeight: 600, border: `1px solid ${C.b1}`,
                borderRadius: 3, background: "transparent", color: C.t2, cursor: "pointer",
              }}>Clear</button>
            </div>
            <div style={{ display: "flex", gap: 6, marginBottom: 8 }}>
              <input value={pantrySearch} onChange={e => setPantrySearch(e.target.value)} placeholder="Search…"
                style={{ flex: 1, padding: "4px 7px", fontSize: 11, background: C.bg, border: `1px solid ${C.b1}`, borderRadius: 3, color: C.t1, outline: "none" }} />
              <select value={pantryFilterSlot} onChange={e => setPantryFilterSlot(e.target.value)}
                style={{ padding: "4px 5px", fontSize: 10, background: C.bg, border: `1px solid ${C.b1}`, borderRadius: 3, color: C.t2 }}>
                <option value="ALL">All slots</option>
                {pantrySlotKeys.map(k => <option key={k} value={k}>{FUNCTIONAL_SLOTS[k].name}</option>)}
              </select>
            </div>
            {filteredIngredients.map(g => {
              const owned = pantrySet.has(g.id);
              return (
                <div key={g.id} onClick={() => togglePantryItem(g.id)} style={{
                  padding: "4px 6px", cursor: "pointer", borderRadius: 3, marginBottom: 1,
                  display: "flex", alignItems: "center", gap: 6,
                  background: owned ? C.acc + "08" : "transparent",
                }}
                  onMouseOver={e => e.currentTarget.style.background = owned ? C.acc + "15" : C.s3}
                  onMouseOut={e => e.currentTarget.style.background = owned ? C.acc + "08" : "transparent"}>
                  <input type="checkbox" checked={owned} readOnly style={{ accentColor: C.acc, cursor: "pointer", margin: 0 }} />
                  <div style={{ width: 5, height: 5, borderRadius: "50%", background: g.phase === "dry" ? C.dry : C.wet, opacity: 0.7 }} />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <span style={{ fontSize: 10, fontWeight: owned ? 600 : 400, color: C.t1 }}>{g.name}</span>
                    <span style={{ fontSize: 9, color: C.t3, marginLeft: 6 }}>{FUNCTIONAL_SLOTS[g.slot]?.name}</span>
                  </div>
                </div>
              );
            })}
      ...
      {showDraftBanner && (
        <div style={{ padding: "8px 14px", borderBottom: `1px solid ${C.info}40`, background: C.infoBg, display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 11, color: C.info, flex: 1 }}>You have an unsaved formula from your last session.</span>
          <button onClick={loadDraft} style={{
            padding: "3px 10px", fontSize: 10, fontWeight: 600, border: "none",
            borderRadius: 3, background: C.info, color: "#fff", cursor: "pointer",
          }}>Resume</button>
          <button onClick={dismissDraft} style={{
            padding: "3px 10px", fontSize: 10, fontWeight: 600, border: `1px solid ${C.info}`,
            borderRadius: 3, background: "transparent", color: C.info, cursor: "pointer",
          }}>Dismiss</button>
        </div>
      )}
          ...
          <div style={{ padding: "10px 14px 6px", borderBottom: `1px solid ${C.b1}`, background: C.s1 }}>
            ...
            <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
              {sortedTemplates.map(t => {
                const comp = templateCompleteness && templateCompleteness.find(c => c.id === t.id);
                const badgeColor = comp ? (comp.fraction >= 1 ? C.acc : comp.fraction > 0.5 ? "#b87a3a" : C.err) : null;
                const dimmed = comp && comp.fraction <= 0.5;
                return (
                  <button key={t.id} onClick={() => { loadTemplate(t); setSelGoal(null); }}
                    title={comp ? `${t.source}: ${t.description} — You own ${comp.owned}/${comp.total} ingredients` : `${t.source}: ${t.description}`}
                    style={{
                    ...
                    {t.name}
                    {comp && <span style={{ marginLeft: 4, fontSize: 9, fontWeight: 600, color: badgeColor }}>({comp.owned}/{comp.total})</span>}
                  </button>
                );
              })}
            </div>
              ...
              {available.length === 0 ? <div style={{ fontSize: 10, color: C.t3, padding: "6px 0" }}>No matches.</div> :
                available.map(g => (
                  <div key={g.id} onClick={() => addIng(g.id)} style={{ padding: "5px 6px", cursor: "pointer", borderRadius: 3, marginBottom: 1, display: "flex", alignItems: "center", gap: 6 }}
                    ...
                    {pantry.length > 0 && (
                      <span style={{ fontSize: 8, fontWeight: 500, color: pantrySet.has(g.id) ? C.acc : C.t3, marginRight: 4, opacity: 0.8 }}>
                        {pantrySet.has(g.id) ? "owned" : "not owned"}
                      </span>
                    )}
